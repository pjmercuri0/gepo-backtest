"""Market gap on the current S&P 100 (Wikipedia constituents table, edited 2026-09-09; user 2026-09-16), GOOG dropped
(same company as GOOGL) = 100 names. Also re-reports the 93-stock repo-list version. Walk-forward refit per year and
...
U93 = tuple(sorted(t for t in open(sys.argv[1]).read().split() if t != 'GOOG'))
OUT93 = 'output/market_gap_backtest_sp100.parquet'
OUT_REPO93 = 'output/market_gap_backtest_93stocks.parquet'
U83, FIT83, SER83 = ec.GAP_UNIVERSE, dict(ec.GAP_FIT), rec.GAP_SERIES
print(f'S&P 100 universe: {len(U93)} names; in 83 but not 93: {sorted(set(U83) - set(U93))}; new: {sorted(set(U93) - set(U83))}', flush=True)
...
if not os.path.exists(OUT93):
    ...
    for i, t in enumerate(U93, 1):
        ...
        if len(y) < 60: print(f'  {t}: only {len(y)} bars total', flush=True)
...
print('names per day by year (sp100 set):', g93.n_names.groupby(g93.index.year).median().to_dict())
print('corr(83-name gap, S&P100 gap) by year:', {y: round(g.g83.corr(g.g93), 3) for y, g in j.groupby(j.index.year)}, '| names per day 93-set median', int(j.n93.median()), flush=True)
...
print('day-level corr(market gap, mean fwd move) by year, S&P 100:',
...
rec.GAP_SERIES = OUT_REPO93; T2 = fmg.training_frame(); FITR = {Y: fmg.fit(T2, Y)[:3] for Y in (2021, 2022, 2023, 2024, 2025, 2026)}
CONF = {'canon': (0.0, FIT83, SER83), 'gap83': (1.0, FIT83, SER83), 'gap93': (1.0, FITR, OUT_REPO93), 'gapSP100': (1.0, FIT93, OUT93)}
rows = []
...
for col in ('pnl', 'sh', 'dd', 'win', 'winp', 'n'):
    P = X.pivot(index='period', columns='config', values=col).reindex(order)[['canon', 'gap83', 'gap93', 'gapSP100']]
    print(f'\n--- {col} ---'); print(P.to_string())
X.to_csv('research/ta_direction/ta_gap_sp100_results.csv', index=False)
