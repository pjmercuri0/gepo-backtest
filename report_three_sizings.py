"""Equity curve with 3 sizing variants: qty1, qty2, qtyx (dyn10k=1 contract per $10k bankroll).
Canonical (as of 2026-06-03): forward empirical DKL (put/call split), rolling 50w P_hist,
GROUND threshold 0.030 across Mon/Tue/Wed/Thu, top-5 per day-of-week, k=30.
"""
...
MIN_ENTRY_DATE = pd.Timestamp('2020-01-01')  # canonical 2026-06-05 (incl. COVID; rv_vs_iv DKL doesn't need pool history, RV needs 10d)
...
def realize(scored, df_idx_view, expiry_close):
    ...
    ok['credit'] = ok['raw_last'] * 0.80  # canonical 2026-06-05: 20% haircut on clamped LAST (was 15%, lowered for fill-quality buffer)
...
CACHE_PATH = f'output/picks_cache_k{K_VAL}_grv_rv_vs_iv_thr075.parquet'  # canon 2026-06-09 (G_rv probs; old delta-prob cache kept at picks_cache_k10_rv_vs_iv_thr075.parquet)
if os.path.exists(CACHE_PATH):
    ...
    print(f'Building picks for k={K_VAL}, per-day thresh, Mon/Tue/Thu (no Wed)...')
    all_picks_per_year = []
    for year in YEARS:
        print(f'── {year} ──', flush=True)
        scored, df_idx_view, expiry_close = score_year(year)
        all_picks_per_year.append(realize(scored, df_idx_view, expiry_close))
...
payload = {
    'config': {
        ...
        'selection':  f'top-5 per day, k={K_VAL}, GROUND threshold 0.075 (all days)',
        'scoring':    'G_rv: RV-implied N(d2) probs in G (canon 2026-06-09); rv_vs_iv DKL (BS d2, 10d RV vs IV); LAST credit (clamped to BBO)',
