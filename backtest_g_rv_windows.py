"""RV window sweep for the G_rv variant. Close-to-close RV over {5,10,15,
20,30} trading days from Yahoo closes (data/daily_bars_yahoo), used in BOTH
G's Kelly probs and DKL's P side (coherent belief). Sanity: Yahoo 10d vs
cached rv_30d (which holds 10d values). Universe restricted to rows with
coverage at all windows. Canon + G_rv(cached rv) as controls. k=10,
count-matched, 0.80 x raw_last. Read-only.
"""
import glob, math, os
import numpy as np
import pandas as pd
...
for p in sorted(glob.glob('data/daily_bars_yahoo/*.csv')):
    ...
    for w in WINDOWS:
        d[f'rv{w}'] = r.rolling(w, min_periods=max(3, w // 2)).std() * np.sqrt(252)
    d['ticker'] = t
    frames.append(d[['ticker', 'date'] + [f'rv{w}' for w in WINDOWS]])
...
df = df.merge(tab, left_on=['ticker', 'entry_date'], right_on=['ticker', 'date'],
              how='left')
need = [f'rv{w}' for w in WINDOWS]
df = df[df[need].notna().all(axis=1)].copy()
print(f'{len(df)} candidates with full RV-window coverage')
print('sanity yahoo rv10 vs cached rv_30d: ratio med=%.3f corr=%.3f' % (
    (df.rv10 / df.rv_30d).median(), df.rv10.corr(df.rv_30d)))
...
arms = {}
# control: G_rv on cached rv_30d with cached DKL
rv_c = np.clip(df['rv_30d'].to_numpy(float), 0.05, 2.0)
Pp, Pq, Pr = nd2_legs(rv_c, rv_c)
df['G_rv_cached'] = kelly_G(Pp, Pq, Pr)
df['SC_cached'] = df['G_rv_cached'] / np.exp(K_CANON * df['DKL'])
arms['rv10_cached'] = 'SC_cached'

for w in WINDOWS:
    rvw = np.clip(df[f'rv{w}'].to_numpy(float), 0.05, 2.0)
    Pp, Pq, Pr = nd2_legs(rvw, rvw)
    g = kelly_G(Pp, Pq, Pr)
    dk = dkl3(Pp, Pq, Pr, Qp, Qq, Qr)
    col = f'SC_{w}'
    df[col] = g / np.exp(K_CANON * dk)
    arms[f'rv{w}d_yahoo'] = col
...
def show(label, s, extra=''):
    print(f'{label:30s} n={s["n"]:5d} avg=${s["avg"]:7.2f} total=${s["tot"]:9.0f} '
...
print('\n=== TRAIN 2020-2024 | TEST 2025 | FULL count-matched ===')
