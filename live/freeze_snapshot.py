...
def _latest_hhmm(latest: dict, fallback: datetime) -> str:
    """Return the ranked scan label, independent of completion/repair time."""
    stem = Path(latest.get("snapshot_file") or "").stem
    if len(stem) == 4 and stem.isdigit():
        return f"{stem[:2]}:{stem[2:]}"
    snapshot_ts = latest.get("snapshot_ts") or ""
    if len(snapshot_ts) >= 16 and snapshot_ts[10] == "T":
        return snapshot_ts[11:16]
...
def maybe_freeze(latest_path: Path | None = None, now: datetime | None = None) -> int:
    ...
    if existing is not None and (existing.get("top_picks") or []):
        ...
        seen = {_pick_key(p) for p in existing_picks}
        additions = []
