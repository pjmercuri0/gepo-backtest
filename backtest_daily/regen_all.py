...
VARIANTS = [
    ("daily-qty1-2022",   "2022-01-01", "2022-12-31", "1", "mid"),
    ("daily-qty2-2022",   "2022-01-01", "2022-12-31", "2", "mid"),
    ("daily-qtyx-2022",   "2022-01-01", "2022-12-31", "dyn10k", "mid"),
]

OOT_DATE = pd.Timestamp("2026-05-04")


def setup_canonical_config(start, end, sizing, basis="mid"):
    """Per-variant overrides only. All other knobs come from config.py canonical defaults."""
    config.START_DATE   = start
    config.END_DATE     = end
    config.SIZING       = sizing
    config.CREDIT_BASIS = basis
...
def run_variant(suffix, start, end, sizing, basis, df_full, expiry_prices):
    print(f"\n========= {suffix}  ({start} → {end}, qty={sizing}, basis={basis}) =========",
          flush=True)
    setup_canonical_config(start, end, sizing, basis)
    setup_filters()
...
def main():
    ...
    for suffix, start, end, sizing, basis in VARIANTS:
        run_variant(suffix, start, end, sizing, basis, df_full, expiry_prices)
