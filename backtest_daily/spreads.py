...
def _build_spread(opts: pd.DataFrame, ticker: str, entry_date,
    ...
    # Credit pricing basis is config.CREDIT_BASIS:
    #   "mid"     = (short_mid - long_mid) — optimistic, matches market mid
    #   "mid80"   = 0.8 × mid — flat-percentage haircut matching the live
    #               execution-floor rule (accept fills at 80% of mid)
    #   "midmid"  = midpoint of mid and natural — pragmatic ~realistic fill
    #   "natural" = (short_bid - long_ask) — worst-case fillable on combo cross
    # Selection, credit_ratio, theta_credit_ratio, etc. all use this basis.
    ...
    nat_credit = short_row["BidPrice"] - long_row["AskPrice"]
    basis = getattr(config, "CREDIT_BASIS", "mid")
    if basis == "natural":
        net_credit = round(nat_credit, 4)
    elif basis == "midmid":
        net_credit = round((mid_credit + nat_credit) / 2.0, 4)
    elif basis == "mid80":
        net_credit = round(mid_credit * 0.8, 4)
    else:
        net_credit = round(mid_credit, 4)
...
def calc_pnl(spot: float, sp: float, bp: float,
             ...
             spread_type: str) -> float:
    """True piecewise-linear spread payoff at expiry. See spreads.calc_pnl
    for full rationale; this is the synchronized copy for the daily backtest."""
    if spread_type == "bull_put":
        if spot >= sp:
