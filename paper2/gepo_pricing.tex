...
Hansen-Sargent formulation, the worst-case $p$ is found endogenously
by the agent. In our formulation, $p$ is given exogenously (by market
implied volatilities) and is itself the object of distrust: the agent
treats the implied distribution as a perturbation away from the
uniform reference and pays a KL-divergence cost proportional to that
deviation. The exact correspondence between the two formulations is
established by the Donsker-Varadhan variational representation of
relative entropy.
...
\begin{proof}
The Donsker-Varadhan lemma states that for any random variable
$\xi$ on a probability space with reference measure $\PP$ and any
$a > 0$,
\begin{equation}
\ln \E_{\PP}[\exp(a \xi)] = \sup_{\Q \ll \PP}
   \Bigl\{ a \E_\Q[\xi] - \KL_e(\Q \| \PP) \Bigr\},
\label{eq:dv-classical}
\end{equation}
with $\KL_e$ denoting the natural-log (nats) relative entropy and the
sup attained at the exponentially tilted measure
$\Q^\star(\omega) \propto \PP(\omega) \exp(a \xi(\omega))$. On the
discrete simplex $\Delta_n$ this is finite-dimensional and standard;
see \citet[Lemma 4.4]{cover2006}.
...
\ln \E_{u_n}[\exp(\theta Z \ln n)]
  = \sup_p \Bigl\{ \theta \ln n \cdot \E_p[Z] - \KL_e(p \| u_n) \Bigr\}.
\end{equation*}
Divide by $\theta$:
\begin{equation*}
V_\theta(Z; u_n) = \sup_p
   \Bigl\{ (\ln n) \E_p[Z] - \tfrac{1}{\theta} \KL_e(p \| u_n) \Bigr\}.
\end{equation*}
Convert relative entropy from nats to trits via
$\KL_e = (\ln n) \KL_n$ and observe that $Z$ is already in trits, so
$\ln n \cdot \E_p[Z]$ has natural-log units; dividing through by
$\ln n$ restates the optimisation in trit units:
\begin{equation*}
\tfrac{1}{\ln n} V_\theta(Z; u_n) = \sup_p
   \Bigl\{ \E_p[Z] - \tfrac{1}{\theta \ln n} \cdot (\ln n) \KL_n(p \| u_n) \Bigr\}
   = \sup_p \Bigl\{ \E_p[Z] - \tfrac{1}{\theta} \KL_n(p \| u_n) \Bigr\}.
\end{equation*}
Setting $k = 1/\theta \cdot \ln n$, i.e.\ $\theta = (\ln n)/k$ as
claimed in \eqref{eq:theta-k-map}, gives
$\sup_p \{\E_p[Z] - k \KL_n(p \| u_n)\} = \sup_p J_k(p, x)$, which is
...
\subsection{Continuous-state extension}\label{sec:continuous}

The framework of Sections~\ref{sec:framework}--\ref{sec:pricing}
treats a finite outcome space $\X = \{\omega_1, \ldots, \omega_n\}$.
Vertical credit spreads naturally fit this discretization (three
outcomes), but options whose payoff is continuous in the underlying
(straddles, calendars, naked single-leg positions) require an
extension to a continuous state space. We show here that the
selection-equivalence and optimality structure carry through
\emph{unchanged} when the discrete probabilities are replaced by
densities and the relative entropy by its differential analogue.

Let $\X = \R$ (or any measurable space). Let $u$ denote a reference
density (e.g.\ a standard normal, or the implied risk-neutral density
of a benchmark contract), $p$ the candidate's implied density, and
$x: \R \to \R$ a measurable payoff function. The continuous growth
functional, evaluated at fraction $w \in [0, 1)$, is
\begin{equation}
G(w; p, x) = \int_\R p(s) \log_n\!\bigl(1 + w \, x(s)\bigr) \, ds,
\label{eq:growth-cts}
\end{equation}
and the differential KL divergence from $u$ is
\begin{equation}
\KL(p \| u) = \int_\R p(s) \log_n\!\frac{p(s)}{u(s)} \, ds.
\label{eq:dkl-cts}
\end{equation}
The continuous regularized objective is then the natural extension
of \eqref{eq:Jk}:
\begin{equation*}
J_k(p, x) = G(w^\star; p, x) - k \cdot \KL(p \| u),
\qquad
w^\star = \arg\max_{w \in [0, 1)} G(w; p, x).
\end{equation*}

\begin{proposition}[Continuous-state FOC]\label{prop:cts-foc}
Under the integrability conditions $p \ll u$, $x \in L^\infty(p)$,
and $x > -1$ $p$-almost everywhere, the Gâteaux derivative of
$J_k(p, x)$ with respect to the scalar $w$ at $w^\star$ vanishes if
and only if
\begin{equation}
\int_\R \frac{p(s) \, x(s)}{1 + w^\star \, x(s)} \, ds = 0.
\label{eq:cts-foc}
\end{equation}
\end{proposition}

\begin{proof}
The regularizer $k \KL(p \| u)$ depends only on the density $p$ and
the reference $u$, not on the wager fraction $w$, so
$\partial_w \bigl[ k \KL(p \| u) \bigr] = 0$. The Gâteaux derivative
of $J_k$ with respect to $w$ therefore reduces to the derivative of
$G$:
\begin{equation*}
\partial_w J_k = \partial_w G(w; p, x)
   = \frac{1}{\ln n} \int_\R \frac{p(s) \, x(s)}{1 + w \, x(s)} \, ds.
\end{equation*}
The interchange of $\partial_w$ and the integral is justified by
dominated convergence: $|p(s) x(s) / (1 + w x(s))| \le p(s) \,
|x(s)| / (1 - w \|x\|_\infty)$, which is $p$-integrable on the bounded
support of $x$. Setting the derivative to zero gives
\eqref{eq:cts-foc}. \qed
\end{proof}

\begin{remark}[Structural parity with the discrete case]
The continuous FOC \eqref{eq:cts-foc} is the obvious analogue of the
discrete FOC stated in the proof of Lemma~\ref{lem:wstar}
(Appendix~\ref{app:proofs}). The selection-equivalence theorem
\ref{thm:main}, the limiting cases (Section~\ref{sec:limits}), and
the reservation-price form (Section~\ref{sec:pricing}) all extend
verbatim with $\sum$ replaced by $\int$ and the discrete simplex
$\Delta_n$ replaced by the space of densities absolutely continuous
with respect to $u$. The Donsker-Varadhan derivation of
Theorem~\ref{thm:multiplier} also generalizes: the DV lemma holds for
arbitrary probability spaces \citep{cover2006}. What does \emph{not}
trivially extend is the closed-form quadratic for $w^\star$ — for a
general continuous payoff, \eqref{eq:cts-foc} is solved
numerically. The GROUND \emph{ranking score}, by contrast, remains a
scalar.
\end{remark}
...
\eqref{eq:growth-cts}, the integrand
