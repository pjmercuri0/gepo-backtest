"""Leave-one-year-out test of empirical lookup vs delta.
For each test year, build the empirical table from the OTHER 3 years.
"""
import sys, math, os
...
def build_table_for(train_years):
    """Build empirical (DTE, delta, IV) → P(ITM) table from given years."""
    frames = []
...
print(f'\n══ LOYO test: Delta vs Empirical (leave-one-year-out) ══')
print(f'{"year":<5} {"variant":<10} {"trades":>5} {"profit":>9} {"$/tr":>7} {"win %":>6} {"Sharpe":>7}')
print('-'*60)

totals_delta = []; totals_emp = []
for test_year in YEARS:
    train_years = [y for y in YEARS if y != test_year]
    print(f'\n── Training table on {train_years} ──', flush=True)
    ...
    ok_d = select_realize(sc, dv, ec)
    n,tot,mu,win,sh = stats(ok_d)
    print(f'{test_year:<5} {"Delta":<10} {n:>5} ${tot:>+7,.0f} ${mu:>+5.2f} {win:>5.1f}% Sh{sh:>+5.2f}')
    totals_delta.append(ok_d)
    ...
    print(f'{test_year:<5} {"Empirical":<10} {n:>5} ${tot:>+7,.0f} ${mu:>+5.2f} {win:>5.1f}% Sh{sh:>+5.2f}')
    totals_emp.append(ok_e)


print(f'\n══ Combined across all 4 years ══')
all_d = pd.concat(totals_delta, ignore_index=True)
all_e = pd.concat(totals_emp, ignore_index=True)
n,tot,mu,win,sh = stats(all_d)
print(f'{"Delta":<10} {n:>5} ${tot:>+7,.0f} ${mu:>+5.2f} {win:>5.1f}% Sh{sh:>+5.2f}')
n,tot,mu,win,sh = stats(all_e)
print(f'{"Empirical":<10} {n:>5} ${tot:>+7,.0f} ${mu:>+5.2f} {win:>5.1f}% Sh{sh:>+5.2f}')
