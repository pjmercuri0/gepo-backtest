...
($\Gamma = G \cdot 3^{-k\,D_{\mathrm{KL}}}$) and freeze the amplification factor
...
\emph{expected return} (Kelly-optimal log-growth), while
...
The companion theoretical paper \citep{mercurio2026pricing} states the
intrinsic GROUND ratio in its general $n$-state form
$\Gamma_k(p, x) = G(p, x) \cdot n^{-k\,D_{\mathrm{KL}}(p \| u_n)}$.
The present paper specializes that formula to $n = 3$ throughout, where
the three states correspond to the three possible held-to-expiry
outcomes of a vertical credit spread: full-win (underlying expires
outside the short strike), partial (underlying expires between the two
strikes), and full-loss (underlying expires beyond the long strike).
Reading the empirical $\Gamma_a = G_a \cdot 3^{-k D_{\mathrm{KL}}^{(a)}}$ as
the $n = 3$ specialization of the theoretical $\Gamma_k$ is the
intended link between the two papers.
...
short-leg open-interest minimum of $10$ contracts, a liquidity gate
that screens out strikes whose quotes may be stale or non-tradable.
...
We instead adopt the \emph{intrinsic} v2 form:
\begin{equation}
    \Gamma_a \;=\; G_a \cdot 3^{-k\,D_{\mathrm{KL}}^{(a)}},
    \qquad k = 20.
    \label{eq:ground-v2}
\end{equation}
...
outcomes: full-win, partial, and full-loss. GROUND is therefore
naturally expressed in log-base-3 (trits), under which the
uniform-outcome entropy $H(\mathbf{u}) = \log_3 3 = 1$ and the
divergence from uniform satisfies $D_{\mathrm{KL}} \in [0, 1]$. The
upper bound has direct interpretation: $D_{\mathrm{KL}} = 1$
corresponds to a candidate concentrating all probability on a single
outcome. We compute $G$, $D_{\mathrm{KL}}$, and the multiplicative
risk discount factor all in base 3, which keeps the exponent
dimensionally consistent. Any other log base produces an
algebraically equivalent ranker at a re-scaled $k$, so the choice of
base is purely a unit convention.
...
holdout 2025--2026     & 350     & \$21{,}511 & 2.47 & $-11.3\%$ & 14.37\% & 8.67\% \\
...
\end{equation}
Here $\gamma_3$ is the sample skewness of the weekly returns ---
negative when there are frequent small wins and occasional large
losses, as is characteristic of short-volatility strategies --- and
$\gamma_4$ is the sample excess kurtosis, which is positive when the
return distribution has fat tails relative to a Gaussian. Both
quantities show up with signs that \emph{enlarge}
$\mathrm{Var}(\widehat{\mathrm{SR}})$ in our setting: the
$-\gamma_3 \mathrm{SR}$ term is positive because both $-\gamma_3$ and
$\mathrm{SR}$ are positive, and the $\tfrac{1}{4}\gamma_4 \mathrm{SR}^2$
term is positive because $\gamma_4$ is positive. The net effect is
that the Mertens intervals are \emph{wider} than the Lo intervals on
this strategy, which is the conservative direction; reporting the
wider interval is the honest choice.
...
yield holds 8.06\% (1\textcent) and 6.26\% (2\textcent).
...
Equation~\ref{eq:ground-v2} is where the empirical edge appears.
...
Under the intrinsic v2 form $\Gamma = G \cdot 3^{-k\,D_{\mathrm{KL}}}$
...
\label{eq:G-used}
\end{equation}
computed in base 3 as discussed in Section~\ref{sec:ground-v2} so that
...
\bibitem[Mercurio(2026b)]{mercurio2026pricing}
P.\,J.~Mercurio.
\newblock An entropy-regularized growth-optimal pricing functional for
discrete-payoff contingent claims in incomplete markets.
\newblock Companion theoretical paper, 2026.
