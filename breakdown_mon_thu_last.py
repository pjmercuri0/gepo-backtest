"""LAST-scored canon, Mon vs Thu breakdown across 0.80×, 0.90×, 1.00× LAST.
Reuses the cache structure from sweep_last_multipliers.py.
"""
...
print('\n══ Mon-only ══')
print(f'{"multiplier":<12} {"year":<6} {"trades":>5} {"profit":>9} {"$/tr":>8} {"win %":>7} {"Sharpe":>9}')
print('-'*64)
for m in MULTIPLIERS:
    mon_total = []
    for year in YEARS:
        scored, df_idx_view, expiry_close = year_cache[year]
        ok = filter_and_pnl(scored, df_idx_view, expiry_close, m)
        mon = ok[ok['dow']==0]
        n,tot,mu,win,sh = stats(mon)
        print(f'{m:<12.2f} {year:<6} {n:>5} ${tot:>+7,.0f} ${mu:>+6.2f} {win:>6.1f}% Sh{sh:>+5.2f}')
        mon_total.append(mon)
    all_mon = pd.concat(mon_total, ignore_index=True)
    n,tot,mu,win,sh = stats(all_mon)
    print(f'{m:<12.2f} {"ALL":<6} {n:>5} ${tot:>+7,.0f} ${mu:>+6.2f} {win:>6.1f}% Sh{sh:>+5.2f}')
    print('-'*64)
...
for m in MULTIPLIERS:
    ...
    for year in YEARS:
        ...
        ok = filter_and_pnl(scored, df_idx_view, expiry_close, m)
        thu = ok[ok['dow']==3]
        n,tot,mu,win,sh = stats(thu)
        print(f'{m:<12.2f} {year:<6} {n:>5} ${tot:>+7,.0f} ${mu:>+6.2f} {win:>6.1f}% Sh{sh:>+5.2f}')
        thu_total.append(thu)
    all_thu = pd.concat(thu_total, ignore_index=True)
    n,tot,mu,win,sh = stats(all_thu)
    print(f'{m:<12.2f} {"ALL":<6} {n:>5} ${tot:>+7,.0f} ${mu:>+6.2f} {win:>6.1f}% Sh{sh:>+5.2f}')
    print('-'*64)
