"""2D (k, thr) sweep for DKL_REFERENCE="rv_vs_iv".

Step 1 (slow ~25 min): score ALL candidates 2020-2025 with rv_vs_iv DKL,
cache (G, DKL, realized PnL) to output/sweep_rv_vs_iv_scored.parquet.

Step 2 (fast): for each (k, thr) combo, recompute GROUND = (exp G - 1) · exp(-k · DKL),
filter top-5 per dow above thr, simulate qty=2.
"""
import sys, math, os
...
ACTIVE_DOWS = [0, 1, 2, 3]
MIN_ENTRY_DATE = pd.Timestamp('2020-04-01')

SCORED_CACHE = 'output/sweep_rv_vs_iv_scored.parquet'

K_GRID   = [10, 25, 50, 100, 200, 400, 800]
THR_GRID = [0.005, 0.010, 0.020, 0.040, 0.075, 0.150, 0.300]

# Per-(Symbol, DataDate) lookups for IV-rank + RV
IV_RANK_LOOKUP = None
try:
...
def build_full_cache():
    ...
    print(f'Loaded pool: {len(pool):,} rows', flush=True)
    parts = []
    for y in YEARS:
...
def eval_combo(scored, k, thr, spy_days):
    ...
    if top.empty:
        return {'k':k,'thr':thr,'n_picks':0,'pnl':0,'final':START_BANKROLL,
                'sharpe':0,'sharpe_w':0,'max_dd':0,'yield':0}
    daily = simulate(top)
...
def main():
    ...
    print(f'\n══ k × thr sweep — DKL=rv_vs_iv (qty=2, 2020-04 → 2025-12) ══')
    ...
    for k in K_GRID:
        for thr in THR_GRID:
            rows.append(eval_combo(scored, k, thr, spy_days))
    ...
    df.to_csv('output/sweep_rv_vs_iv_results.csv', index=False)
    print(f'\nWrote output/sweep_rv_vs_iv_results.csv')
